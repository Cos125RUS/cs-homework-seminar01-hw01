...
// Проверка на врезаия в фигуры внизу
bool GluingTest(int x, int y, int[,] field, int[,] mapping, int row, int column)
{
    for (int i = 0; i < row; i++)
        for (int j = 0; j < column; j++)
...
while (true)
{
    ...
    if (key == ConsoleKey.Spacebar)
    {
        ...
        if (y > 1)
        {
            while (GluingTest(x, y, field, mapping, row, column))
            {
                y--;
                if (SideTest(x, y, 1, field, mapping, row, column)) x--;
                if (SideTest(x, y, -1, field, mapping, row, column)) x++;
            }
        }
        Figure(x, y, mapping, row, column);
